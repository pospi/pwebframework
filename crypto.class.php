...
class Crypto
{
	...
	public static function encryptString($string, $recipientEmail, $outputFile = null)
	{
		...
		if ($outputFile) {
			return self::exec("echo $string | " . self::$GPG_PATH . " --yes -e -r $recipientEmail -o $outputFile");
		}
		return bin2hex(shell_exec("echo $string | " . self::$GPG_PATH . " --yes -e -r $recipientEmail"));
	}
	...
	public static function encryptFile($file, $recipientEmail, $targetFilename = false)
	{
		...
		if (self::exec(self::$GPG_PATH . " --yes -e -r $recipientEmail" . (!$replaceOriginal && $targetFilename ? " -o $targetFilename" : '') . " " . escapeshellarg($file)) === 0) {
			if ($replaceOriginal) {
				unlink($file);
	...
	public static function decryptFile($file, $keyPassphrase = null, $unencryptedFile = null)
	{
		...
		$method = $unencryptedFile ? array('Crypto', 'exec') : 'shell_exec';

		if ($keyPassphrase) {
			$res = call_user_func($method, "echo $keyPassphrase | " . self::$GPG_PATH . " --yes --passphrase-fd 0 " . ($unencryptedFile ? "-o $unencryptedFile " : '') . "-d $file");
		} else {
			$res = call_user_func($method, self::$GPG_PATH . " --yes " . ($unencryptedFile ? "-o $unencryptedFile " : '') . "-d $file");
		}
