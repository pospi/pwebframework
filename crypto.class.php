...
class Crypto
{
	...
	public static function encryptString($string, $recipientEmail, $outputFile = null)
	{
		...
		if ($outputFile) {
			$cmd = "echo $string | " . self::$GPG_PATH . " --yes -e -r $recipientEmail -o $outputFile";
			self::log("RUNNING ENCRYPTION: " . $cmd, true);

			return self::exec($cmd);
		}

		$cmd = "echo $string | " . self::$GPG_PATH . " --yes -e -r $recipientEmail";
		self::log("RUNNING ENCRYPTION: " . $cmd, true);
	...
	public static function encryptFile($file, $recipientEmail, $targetFilename = false)
	{
		...
		$cmd = self::$GPG_PATH . " --yes --status-fd 1 -e -r $recipientEmail" . (!$replaceOriginal && $targetFilename ? " -o $targetFilename" : '') . " " . escapeshellarg($file);
		self::log("RUNNING ENCRYPTION: " . $cmd, true);
	...
	public static function decryptFile($file, $keyPassphrase = null, $unencryptedFile = null)
	{
		...
		if ($keyPassphrase) {
			$cmd = ' | ' . self::$GPG_PATH . " --yes --passphrase-fd 0 --status-fd 1 " . ($unencryptedFile ? "-o $unencryptedFile " : '') . "-d $file";
			$logCmd = "echo '[PASSPHRASE]'" . $cmd;
			$cmd = "echo {$keyPassphrase}" . $cmd;
		} else {
			$cmd = self::$GPG_PATH . " --yes " . ($unencryptedFile ? "-o $unencryptedFile " : '') . "-d $file";
			$logCmd = $cmd;
		}
